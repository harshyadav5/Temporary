...
test('renders clicking button decrements counter display', () => {
  ...
  //find and display counter
  const counterDisplay = findByTestAttr(wrapper,'counter-display');
  expect(counterDisplay.text()).toContain(counter-1);
})

test('renders error message on decrementing if counter values is zero', () => {
  const counter = 0;
  const wrapper = setup(null,{counter});
  ...
  expect(errorDisplay.text());
})

test('error message should be removed on incrementing counter when counter is zero', () => {
  const counter = 0;
  const wrapper = setup(null,{counter});
  ...
  //find and display counter value
  const counterDisplay = findByTestAttr(wrapper,'counter-display');
  expect(counterDisplay.text()).toContain(counter+1);
})
